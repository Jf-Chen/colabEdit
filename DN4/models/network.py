...
class FourLayer_64F(nn.Module):
	def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3):
		...
		self.features = nn.Sequential(                              # 3*84*84
            ...
            # in_channels=3,out_channels=kernel_num=64,kernel_size=3
            # 84*84*3 by (3*3)*64 => (84*84)*64 
			nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=use_bias),
			norm_layer(64),
            # 大于0的不变，小于0的乘以系数0.2，True指示改变原输入
    ...
    # input1:query_images,len(input2)=5
	def forward(self, input1, input2):

		# extract features of input1--query image
        # (84*84)*3->(21*21)*64
		q = self.features(input1)
        # 在5-way 5-shot时，input1->torch.Size([50, 3, 84, 84])，q->torch.Size([50, 64, 21, 21])

		# extract features of input2--support set
		S = []
		for i in range(len(input2)):
			support_set_sam = self.features(input2[i]) # [5, 64, 21, 21]
			B, C, h, w = support_set_sam.size()
			support_set_sam = support_set_sam.permute(1, 0, 2, 3) 
            # tensor.permute(),将原本的tensor维度按照括号里的顺序排列，[64,5,21,21]
			support_set_sam = support_set_sam.contiguous().view(C, -1) # [64,2205]
			S.append(support_set_sam)
        # 5-way 5-shot,len(S)=5, S[0]->torch.Size([64, 2205]),
        # len(input2)=5,input2[0]->torch.Size([5, 3, 84, 84])
...
class ImgtoClass_Metric(nn.Module):
	...
	def cal_cosinesimilarity(self, input1, input2):
        # 5-way 5-shot,input1->[50, 64, 21, 21], input2->5*[64,2205]
		...
		for i in range(B):
			...
			query_sam = torch.transpose(query_sam, 0, 1) #转置，[441,64]
			query_sam_norm = torch.norm(query_sam, 2, 1, True) # 求1维度上的2范数->[441,1]
			query_sam = query_sam/query_sam_norm #[441,64]

			if torch.cuda.is_available():
				inner_sim = torch.zeros(1, len(input2)).cuda()
            
            # 5-shot时，input2[j]中是同一类别
			for j in range(len(input2)):
				support_set_sam = input2[j] #[64,2205]
				support_set_sam_norm = torch.norm(support_set_sam, 2, 0, True) # [1,2205]
				support_set_sam = support_set_sam/support_set_sam_norm #[64,2205]
