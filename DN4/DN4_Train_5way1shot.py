...
def train(train_loader, model, criterion, optimizer, epoch_index, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(train_loader):
        ...
        # type(target)=tensor,target.size()=50
		target = target.cuda()

		# Calculate the output
        # input_var1,type = Tensor ,size() = torch.Size([75, 3, 84, 84]),相当于(N*query_num,(image.size())
        # input_var2,type = list, len = 5
        # input_var2[0],size() = torch.Size([1, 3, 84, 84]), 相当于(K,(image.size())
