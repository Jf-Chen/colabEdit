...
def train(args, train_data_loader, validate_data_loader, model, model_save_path):
    ...
    for task_index, task in enumerate(train_data_loader):
        ...
        # 大小
        # images.size()=torch.Size([80, 3, 84, 84])
        # labels.size()=torch.Size([80])
        images, labels = task
        ...
        query_targets = query_targets.cuda(args.devices[0]) # torch.Size([50])
        
        loss = nn.CrossEntropyLoss()(logits, query_targets) # 一个数
